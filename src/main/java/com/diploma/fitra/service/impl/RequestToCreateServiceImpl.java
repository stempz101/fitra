...
import com.diploma.fitra.repo.RequestToCreateRepository;
...
@Slf4j
public class RequestToCreateServiceImpl implements RequestToCreateService {
    ...
    public List<RequestDto> getRequests(Authentication auth) {
        ...
        if (!userDetails.getAuthorities().contains(new SimpleGrantedAuthority("ROLE_ADMIN"))) {
            ...
            requestDtoList = requestToCreateRepository.findAllByTravel_Creator(creator).stream()
                    .map(RequestMapper.INSTANCE::toRequestDto)
                    .collect(Collectors.toList());
        } else {
            requestDtoList = requestToCreateRepository.findAll().stream()
                    .map(RequestMapper.INSTANCE::toRequestDto)
                    .collect(Collectors.toList());
        }
    ...
    public RequestDto getRequest(Long requestId, Authentication auth) {
        ...
        RequestToCreate request = requestToCreateRepository.findById(requestId)
